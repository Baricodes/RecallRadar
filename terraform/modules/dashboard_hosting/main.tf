...
locals {
  bucket_name = var.bucket_name != "" ? var.bucket_name : "${var.project_name}-dashboard-${data.aws_caller_identity.current.account_id}"
}
...
resource "aws_cloudfront_origin_access_control" "dashboard" {
  ...
  description                       = "OAC for RecallRadar dashboard S3 origin"
...
resource "aws_cloudfront_distribution" "dashboard" {
  ...
  comment             = "RecallRadar React dashboard"
